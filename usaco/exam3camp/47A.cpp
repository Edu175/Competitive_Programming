...
const ll MAXN=200,lim=250,C=lim-5,INF=1e15;
int main(){FIN;
	ll n,A,B; cin>>n>>A>>B;
	vector<ll>a(n);
	fore(i,0,n)cin>>a[i];
	vector<pair<ii,ii>>p(n+2);
	p[0]=p[n+1]={{0,0},{0,0}};
	ll flag=1;
	ll g=gcd(A,B);
	...
	fore(i,0,n){
		if((-a[i])%g)flag=0;
		else p[i+1]=diophantine(A,B,-a[i]);
	}
	if(!flag){
		cout<<"-1\n";
		return 0;
...
//	cout<<"\n";
	ll dp[n+5][2*lim];
	mset(dp,0);
	fore(i,1,n+2)fore(j,-C,C){
		ll &res=dp[i][j+C];
		res=INF;
		ii pr=p[i-1].fst+(-C)*p[i-1].snd;
		ii now=p[i].fst+j*p[i].snd;
		fore(k,0,2*C){
