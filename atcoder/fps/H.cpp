...
int main(){FIN;
	...
	ll n,m; cin>>n>>m;
	// ll res=0;
	// fore(k,0,n+m+5)fore(z,0,k+1){
	// 	ll cur=mul(mul(nCr(k-z,n),nCr(m+k-z-1,m)),nCr(k,z));
	// 	res=(z&1?sub:addm)(res,cur);
	// }
	ll tam=n+m+5;
	vv pz(tam),pj(tam);
	fore(z,0,tam){
		pz[z]=fci[z];
		if(z&1)pz[z]=sub(0,pz[z]);
	}
	fore(j,0,tam){
		if(min({m+j-1,j-n,j-1})<0)continue;
		pj[j]=mul(fc[m+j-1],mul(fci[j-n],fci[j-1]));
	}
	
	vv pk=multiply(pz,pj);
	
	ll res=0;
	fore(k,0,tam){
		ll cur=mul(fc[k],pk[k]);
		res=addm(res,cur);
	}
	res=mul(res,mul(fci[n],fci[m]));
	cout<<res<<"\n";
	return 0;
}
