...
void tablero(int N, vector<int> &A, vector<int> &B) {
    ...
    for(int i=0; i<(int)A.size(); i++){
        ...
        nodos_grafo.insert(A[i]);
        nodos_grafo.insert(B[i]);
}
...
int con_quien_comercio(vector<int> &controlados) {
    ...
    set<int>copia_nodos;
    copia_nodos = nodos_grafo;
    for(auto &it:controlados){
        copia_nodos.erase(it);
    }
    int res = 0;
    for(auto &it:copia_nodos){
        for(int i=0; i<grafo[it].size(); i++){
            int vecino = grafo[it][i];
            if(copia_nodos.count(vecino) == 0) return it;
        }
    }
    return -1;
    /*controla.clear();
