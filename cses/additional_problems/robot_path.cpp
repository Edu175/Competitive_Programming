...
struct ln{
	...
	ll dist(pt r, ln l){
		// cout<<"dist "<<r.cv()<<"\n";
		if(seghas(r)&&l.seghas(r))return (r-p).manh();
		return -1;
	}
	ll dist(ln l){
		if(*this/l){ // useless
...
int main(){FIN;
	...
	fore(i,0,n){
		char c; ll d; cin>>c>>d;
		auto w=trad(c);
		if(SZ(mov)&&mov.back().fst==(w^2))break;
	...
	fore(i,0,n){
		...
		// cerr<<l.cv()<<"\n";
		a.pb(l);
		auto halt=[&](set<ii> &st, set<ll> &se, ll l, ll r){ // now [,]
			auto it=st.upper_bound({l,INF});
			if(it!=st.begin()&&prev(it)->snd>=l){
				// cerr<<"halts fst point\n";
				a.pop_back();
				return 1;
				// corto al toque
			}
			// yo contengo algun punto (solo extremos relevantes)
			auto s=se.lower_bound(min(l,r));
			auto e=se.upper_bound(max(l,r));
			if(s!=e){
				ll d=min(abs(*s-l),abs(*prev(e)-l));
				// cerr<<"halts contengo "<<d<<"\n";
				auto &pq=a.back().pq;
				if(pq.x!=0)pq.x=d;
				if(pq.y!=0)pq.y=d;
				acum+=d;
		...
		if(pq.y==0){
			if(halt(hor[p.y],ho[p.y],p.x,q.x))break;
		}
		else {
			if(halt(ver[p.x],ve[p.x],p.y,q.y))break;
		}
		acum+=mov[i].snd;
		p=q;
	}
