...
void dfs(ll x){
	...
	for(auto y:t[x]){
		...
		// cout<<x<<"->"<<y<<": "<<a<<" "<<b<<" "<<w<<": "<<(a*to2(b)+to2(a)*b)*w<<"\n";
	}
}
...
int main(){
	...
	fore(i,0,n){
		...
		// cout<<a[i]<<": ";
		nod[a[i]].pb(i);
		// for(auto d:divs[a[i]])nod[d].pb(i);//,cout<<d<<" ";
		// cout<<"\n";
	}
	vector<ii>ed;
	fore(i,0,n-1){
	...
	virtual_init();
	vv num(MAXV);
	fore(d,1,MAXV)if(mu[d]){
		vv v;
		for(ll m=d;m<MAXV;m+=d)for(auto i:nod[m])v.pb(i);
		tot=SZ(v);
		if(!tot)continue;
		// cerr<<d<<":\n";
	...
	fore(i,1,MAXV)for(ll j=2*i;j<MAXV;j+=i)if(mu[j/i])
		num[i]=(mu[j/i]==1?add:sub)(num[i],num[j]);
	
	// fore(p,2,MAXV)if(cr[p]==p){ // sos dp
	// 	// cerr<<p<<" p\n";
	// 	ll s=(MAXV-1)/p;
	// 	for(ll i=s;i>=0;i--)num[i]=sub(num[i],num[i*p]);
	// }
	ll res=0;
	fore(i,0,MAXV){
		res=add(res,mul(sz[i],num[i]));
		// if(num[i])cerr<<i<<": "<<num[i]<<"\n";
	}
	cout<<res<<"\n";
	// MOD answer
    return 0;
