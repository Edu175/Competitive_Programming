...
int main(){FIN;
	...
	fore(x,0,cnt+1)F[0][x]=x,Fb[0][x]=x;
	fore(k,1,B+1)fore(x,0,cnt+1){
		F[k][x]=F[k-1][F[1][x]];
	}
	fore(i,1,cnt/B+1)fore(x,0,cnt+1){
		Fb[i][x]=Fb[i-1][F[B][x]];
	}
	ll q; cin>>q;
	while(q--){
