...
ll count(vv s){ // s centro
	...
	ll par=SZ(s)>1;
	vv frac(n+1);
	vv sumfrac(n+2),small(n+2);
	fore(i,1,n+1){ // frac n no se calcula
		ll p=i-1;
	...
	fore(i,0,n)cout<<tag[i]<<","<<dis[i]<<" ";;cout<<"\n";
	fore(i,0,n){
		ll c=i+1;
		bool bo=c>=2;
		auto [r,x]=a[i];
		ll cant=tag[x]==-1?1:hist[dis[x]]++-mp[{tag[x],dis[x]}]++;
		ll s1=mul(mul(frac[c],fpow(9,MOD-2)),sub(big[n],big[c]));
		if(!bo)s1=mul(s1,3);
		
		s1=add(s1,mul(r,sub(fpow(2,n),fpow(2,c))));
		
		ll s0=add(small[c-1-bo],mul(r,sumfrac[c-1-bo]));
		
		s0=mul(s0,fpow(2,c-3));
		
		ll resi=add(s0,s1);
		cerr<<c<<": "<<r<<" "<<x<<": "<<s0<<" "<<s1<<": "<<cant<<" "<<resi<<": "<<mul(cant,resi)<<"\n";
