...
ll isa=1;
ii qan(vector<pt>&a,pair<pt,pt>b, ll c){
	ll l=0,r=SZ(a)-1;
	while(l<=r){
		ll m=(l+r)/2;
...
ll cnt(vector<pt>&a,vector<pt>&b,vector<pt>&c,pair<pt,pt>da,pair<pt,pt>db,ll f){ // SZ(vi)>=2
	pair<pt,pt>dc={c[0],c[1]};
	if(c[0].left(da.fst,da.snd)){
		if(c[SZ(c)-1].left(da.fst,da.snd)){
	...
	ii qa=qan(a,dc,!(a[0]==da.fst));
	if(!isa)cout<<"pri "; ii qb=qan(b,dc,(b[0]==db.fst)^1);
	ii qc=qan(c,da,(c[0]==dc.fst));
	if(c[qc.snd]==a[qa.snd])qc.fst--;
	if(!isa){
		cout<<"seg ";
		qc.fst=SZ(c),qa.fst=0,qb=max(qb,qan(b,{dc.snd,dc.fst},(b[0]==db.fst)));
	}
	if(f&&*lower_bound(ALL(c),b[qb.snd])==b[qb.snd]){
		qc.fst--;
		if(!isa)cout<<".";
	}
	if(!isa){
    ...
    for(auto &m:seg){
    	...
    	fore(i,0,SZ(m.snd)){// <= 125 // A
    		...
    		fore(j,0,SZ(m.snd)){ // B
    			...
    			pair<pt,pt> da={a[0],a[1]},db={b[0],b[1]};
				if(!b[0].left(a[0],a[1]))swap(da.fst,da.snd);
				if(a[0].left(b[0],b[1]))swap(db.fst,db.snd);
				if(i==j&&da.fst==db.fst)swap(db.fst,db.snd);
    			fore(h,0,SZ(seg[per(m.fst)])){
    				vector<pt>&c=seg[per(m.fst)][h];
    				isa=0; res=max(res,cnt(a,b,c,da,db,1)); isa=1;
    				//cout<<"c: "; for(auto edu:c)cout<<edu.x<<","<<edu.y<<" "; 
    				//cout<<"\n";
    				ll resi=0;
    				if(i==j)resi=max(cnt(a,a,c,da,da,0),cnt(b,b,c,db,db,0));
    				else resi=cnt(a,b,c,da,db,1);
    				//cout<<"= "<<resi<<"\n\n";
    				res=max(res,resi);
    			}
    		}
