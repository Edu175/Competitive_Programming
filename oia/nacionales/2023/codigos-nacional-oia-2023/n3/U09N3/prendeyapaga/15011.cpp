...
vector<int> hoteles(vector<int>& padre, vector<int>& tipo, vector<int>& nodo) {
    ...
    for(int i = 0; i < tipo.size(); i++) {
        if(tipo[i] == 0) {
            //change to wifi
            if(isRetrans[nodo[i]] == false || nodo[i] == 0) {Q.push_back(0); continue;} //if node is already in Wifi, or is main root.
            ...
            bool sePuedeCambiarDes = dfsDescendente(nodo[i], hijos, isRetrans, false);
