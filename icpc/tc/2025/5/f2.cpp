...
ll dpcoef[35+15][15+2];
// b+fg+m-c-b-fg , b+fg-1
// m-c , b+fg-1
// n-i-1-c , b+fg-1
ll getcoef(ll i, ll b, ll c, ll fg){
	auto &res=dpcoef[i+c][b+fg];
	if(res!=-1)return res;
	ll m=n-i-1;
	return res=nCr(n-i-1-c,b+fg-1);
	// return res=ceb(b+fg,m-c-(b-1)-fg);
}
ll mem[35][15][15];
ll get(ll i, ll s, ll v){
	auto &res=mem[i][s][v];
	if(res!=-1)return res;
	// ll m=n-i-1;
	res=0;
	fore(b,0,s+1)fore(c,b,s+1)fore(fg,0,2){
		ll num=g(s,v,b,0,c,fg),coef=getcoef(i,b,c,fg);
...
int main(){
	...
	while(t--){
		...
		if(1){
			...
			mset(mem,-1);
			mset(dpcoef,-1);
			// fore(i,0,25)fore(j,0,k+1){
			// 	fore(l,0,x+1){
			// 		fore(m,0,i+1)dpf[i][j][l][m]=-1;
			// 		mem[i][j][l]=-1;
			// 	}
			// 	fore(j2,0,k+1)fore(fg,0,2)dpcoef[i+j2][j+fg]=-1;
			// }
			// mset(dpg,-1);
		}
		ll res=f(0,k,x,0);
