...
void expand0(ll x){
	ll ki=k+1;
	fore(i,0,log2techo(ki)){
		ll q=1ll<<i;
		if(2*q>ki)q-=2*q-ki;
		append_right(c[1],x,q);
		append_or(x,x,c[1]);
	}
}
...
ll uno=10;
void compare(ll a, ll b, ll f, ll all){ // f := a > b
	// assumes f[k]=1
	// append_left(c[0],f,k);
	// append_and(a,_a,all);
	append_and(b,b,all);
...
ll all=6;
void compare1(ll _a, ll _b, ll f){ // f := a > b
	// assumes a[k]=1
	append_left(c[0],f,k);
	ll a=c[1],b=c[2];
	append_and(a,_a,all);
...
void ifmaysw(ll p){
	...
	ll f=c[3];
	vector<bool>v(B);
	fore(i,0,B)v[i]=(i%(2*m)==0&&i/k+p<n);
	vector<bool>u(B); //shifted k
	fore(i,0,B-k)u[i+k]=v[i];
	append_store(f,u);
	ll mk=c[4];
	fore(i,0,B)v[i]=v[i]|v[i/k*k];
	append_store(mk,v);
	append_print(f);
	append_print(mk);
