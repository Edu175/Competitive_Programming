...
int main(){FIN;
	...
	while(t--){
		...
		vv bad(n);
		ll did=0;
		for(ll i=n-1;i>=0;i--)did|=ans[i]!=-1,bad[i]=!did;
		ll prev=-1,lm=-1;
		ll typ=0;
		// fore(i,0,n)cerr<<ans[i]<<" ";;cerr<<"\n";
		// fore(i,0,n)cerr<<lam[i]<<" ";;cerr<<"\n";
		// fore(i,0,n)cerr<<bad[i]<<" ";;cerr<<"\n";
		fore(k,0,n){
			if(ans[k]==-1){
				if(prev!=-1&&!bad[k])ans[k]=(prev-k)*lm+ans[prev];
			}
