// see c_charles, this one has a bug in good
#include <bits/stdc++.h>
#define pb push_back
#define fst first
#define snd second
...
poly multiply(poly& p1){
	int n=p1.size()+p1.size()+1;
	int m=1,cnt=0;
	while(m<=n)m+=m,cnt++;
...
bool good(vv a, ll k){
	...
	fore(i,0,MAXV){
		ll j=k-2*i;
		if(j!=i&&j>=0&&j<MAXV)ban+=3*c[i]*c[j];
	}
	auto d=multiply(c);
	ll res=0;
	fore(i,0,MAXV){
		ll j=k-i;
		if(j>=0&&j<MAXV)res+=c[i]*d[j];
	}
	res-=ban;
	return res>0;
}
