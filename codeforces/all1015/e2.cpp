...
struct FFT {
	...
	vector<ll> cv(const vector<uint64_t> &as, const vector<uint64_t> &bs, u *v) {
...
vector<ll> conv_small(const vector<uint64_t> &as, const vector<uint64_t> &bs) {
	static uint32_t v[2<<LG];
	static FFT<uint32_t, uint64_t, 40961, 3> fft;
	auto res=fft.cv(as, bs, v);
	fore(i,0,SZ(res))res[i]%=40961;
	return res;
}
