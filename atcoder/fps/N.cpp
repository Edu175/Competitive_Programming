...
vv get(vv a, ll t, ll sig){ // first t terms of product of 1-x^a[i]
	// might want sig ==-1 so ans*(-1)
	vv oc(t);
	for(auto k:a)if(k<t)oc[k]++;
	vv res(t);
	fore(k,1,t){
		for(ll j=1;j*k<t;j++){
			ll den=mul(fci[j],fc[j-1]);
			res[j*k]=addm(res[j*k],mul(den,oc[k]));
		}
	}
	if(sig==1)for(auto &i:res)i=sub(0,i);
	return res;
}
...
int main(){FIN;
	...
	fore(i,0,n)cin>>a[i];
	vv nump(n),denp(n);
	fore(i,0,n)nump[i]=(i+1)*(a[i]+1),denp[i]=i+1;
	ll t=n+3;
	vv num=get(nump,t,1);
	vv den=get(denp,t,-1);
	vv res=add(num,den);
