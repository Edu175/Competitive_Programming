...
long long superbicis(vv &x, vv &y, vector<int> &c) {
	ll n=SZ(x);
	ll nodes=n;
	MCF fl(nodes+5);
	ll sum=0;
	...
	fore(x,0,n){
		c[x]-=b;
		if(c[x]>0)pos.pb(x),fl.add_edge(nodes,x,c[x],0);
		if(c[x]<0)neg.pb(x),fl.add_edge(x,nodes+1,-c[x],0);
	}
	auto dis=[&](ll i, ll j){return abs(x[i]-x[j])+abs(y[i]-y[j]);};
	for(auto i:pos)for(auto j:neg)fl.add_edge(i,j,INFFLOW,dis(i,j));
	
	return fl.get_flow(nodes,nodes+1).snd;
}
