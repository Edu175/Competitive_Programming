...
int main(){FIN;
	...
	fore(i,0,n){
		ll x; cin>>x; suma+=x;
		fl.add_edge(nodes,i,x,0);
	}
	fore(i,0,n){
		ll x; cin>>x; sumb+=x;
		fl.add_edge(n+i,nodes+1,x,0);
	}
	fore(i,0,n){
		fore(j,0,n){
			ll v; cin>>v;
	...
	auto [mxf,mnc]=fl.get_flow(nodes,nodes+1);
	if(suma==sumb&&suma==mxf){
		cout<<-mnc<<"\n";
		vector<string>res(n,string(n,'.'));
		fore(i,0,n){
			for(auto e:fl.g[i])if(e.f>0)res[i][e.to-n]='X';
		}
