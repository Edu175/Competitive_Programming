...
ll vis[MAXN];
ll n,m;
ll dfs(ll x, ll ori, ll ant){
	vis[x]=true;
	if(s[x] && SZ(g[x])==1 && x!=ori){
		ests[x]=1;
		return 1;
	}
	if(ant==0 && !s[x]){
	...
	for(auto y:g[x]){
		if(vis[y])continue;
		if(!s[x])b=b|dfs(y,ori,s[x]);
		else b=b&dfs(y,ori,s[x]);
	}
	ests[x]=b;
	return b;
}
ll dfd(ll x, ll ori, ll ant){
	vis[x]=true;
	if(d[x] && SZ(g[x])==1 && x!=ori){
		estd[x]=1;
		return 1;
	}
	if(ant==0 && !d[x]){
	...
	for(auto y:g[x]){
		if(vis[y]) continue;
		if(!d[x])b=b|dfd(y,ori,d[x]);
		else b=b&dfd(y,ori,d[x]);
	}
	estd[x]=b;
	return b;
}
...
int main(){
	...
	ll _; cin>>_; while(_--){
		...
		fore(i,0,n){
			if(!vis[i]&&s[i]&&(grad[i]==1)) dfs(i,i,-1);
		}
		mset(vis,0);
		fore(i,0,n){
			if(!vis[i]&&d[i]&&(grad[i]==1)) dfd(i,i,-1);
		}
