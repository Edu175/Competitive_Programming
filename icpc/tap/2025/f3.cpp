...
int main(){
	...
	auto _fills=[&](ln a, ln b, ld r0, ld r1)->bool {
		if(r0<a.pq.norm())swap(a,b),swap(r0,r1);
		if(r0<a.pq.norm())return 0;
		if(r1>=b.pq.norm())return 1;
		pt p=b.p+b.pq*(r1/b.pq.norm());
		return (p-a.p).norm()<=r0;
	};
	auto fills=[&](ln a, ln b, ld r0, ld r1)->bool {
		ld ret=_fills(a,b,r0,r1);
		cerr<<"fills "<<a.cv()<<" "<<b.cv()<<" "<<r0<<" "<<r1<<": "<<ret<<"\n";
		return ret;
	};
	for(auto [_r,idx]:qs){
		r=_r;
		...
		while(gotall){
			cout<<"\n\n\nwhile\n";
			cout<<"a: ";
			for(auto i:a)cout<<i.cv()<<" ";;cout<<"\n";
			cout<<"b: ";
			for(auto i:b)cout<<i.cv()<<" ";;cout<<endl;
			assert(min(SZ(a),SZ(b))>=2);
			elem xA=a.back();
			elem xB=b.back();
			ln lA(a.end()[-2].p, xA.p);
			ln lB(b.end()[-2].p, xB.p);
			if(lA/lB){
				cout<<"paralelas\n";
				break;
			}
			...
			ld r1=r-xB.s;
			// cout<<"sorongo\n";
			if(!fills(lA,lB,r0,r1)){
				cout<<"not fills\n";
				break;
			}
			fij+=area({lA.p,lB.p,p});
			cerr<<"added "<<area({lA.p,lB.p,p})<<"\n";
			ld d0=xA.s+lA.pq.norm();
			ld d1=xB.s+lB.pq.norm();
			elem nue({p,-1,-1,-1});
			sac(0); sac(1);
			if(d0<d1){
				cout<<"va a A\n";
				nue.s=d0;
				nue.ang=angle(p-a.back().p, b.back().p-p);
				agr(0,nue);
			}
			else {
				cout<<"va a B\n";
				nue.s=d1;
				nue.ang=angle(p-b.back().p, a.back().p-p, 1);
				agr(1,nue);
			}
		}
		...
		cerr<<"sac: "<<eA.ang-sacA<<" "<<eB.ang-sacB<<"\n";
