...
const ll MAXN=1e5+5;
unordered_map<int,ll> ft[MAXN];


void upd(int i0, int j0){
	cout<<"upd "<<i0<<","<<j0<<"\n";
...
ll get(ll i0, ll j0){
	...
	for(int j=j0;j>0;j-=j&-j)
		res+=ft[i][j];
	return res;
}
ll query(ll i0, ll i1, ll j0, ll j1){
	cout<<"query "<<i0<<","<<i1<<" "<<j0<<","<<j1<<"\n";
...
int main(){FIN;
	...
	imp(fb)
	imp(fs);
	fore(i,0,n)fs[i]=fr[i]==n?n:fs[fr[i]];
	 imp(fs) imp(fr)
	ll q; cin>>q;
	vector<ii>qu(q);
	vector<ll>ans(q);
	...
	// STree st(n+5);
	fore(i,0,n){
		if(fb[i]==-1)upd(i,fs[i]);
		else upds[fb[i]].pb(i);
	}
	fore(i,0,n+1){
		for(auto [r,id]:querys[i]){
			ans[id]=query(qu[id].fst,qu[id].snd,r,n+2);
		}
		for(auto p:upds[i])upd(p,fs[i]);
	}
