...
ll f(ll n){
	...
	fore(m,1,n+1){
		...
		ll ed=m*(m-1)*fpow(2,MOD-2)%MOD;
		ed=(ed-m-1+MOD)%MOD;
		res=(res+ed*fpow(m,m-3)%MOD*f(n-m))%MOD;
	}
	return res;
}
...
ll f2(ll n){
	...
	fore(m,1,n+1){
		ll ed=m-1;
		res=(res+ed*2*fpow(m,m-3)%MOD*f2(n-m))%MOD;
	}
...
int main(){FIN;
	...
	mset(dp,-1);
	ll r2=f2(n);
	//cout<<(r1+r2)<<" "<<fc[n]<<" "<<fpow(2,n)<<": ";
	ll res=(r1+r2)*fc[n]%MOD*fpow(2,n)%MOD;
	cout<<res<<"\n";
	return 0;
}
