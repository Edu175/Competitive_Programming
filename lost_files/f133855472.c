...
tf mulmod(tf a, tf b){
	return bint(a)*bint(b)%MOD;
}
tf addmod(tf a, tf b){return (a+b)%MOD;}
tf submod(tf a, tf b){return (a-b+MOD)%MOD;}
struct CD {
	tf x;
	CD(tf x):x(x){}
	CD(){}
...
poly be(poly b, ll e){
	poly res={1};
	while(e){
		//imp(res); imp(b); cout<<e<<"e\n\n";
		if(e&1)res=down(multiply(res,b));
		b=down(multiply(b,b)),e/=2;
	}
	return res;
}
...
int main(){FIN;
	...
	poly res=be(p,k);
	//imp(res);
	fore(i,0,SZ(res))if(res[i])cout<<i<<" ";
	cout<<"\n";
	return 0;
}
