...
void dfs2(ll x){
	fore(j,0,SZ(g[x])){
		...
		if(F[0][x]!=-1)ch[y]=merge(ch[y],up(ch[x],wf[x]));
		dfs2(y);
	}
}
ll val[MAXN];
...
int main(){FIN;
	...
	fore(x,0,n){
		...
		fore(j,1,m+1){
			auto [y,w]=g[x][j-1];
			p[j]=p[j-1];
			if(y!=F[0][x])p[j]=merge(p[j],up(h[y],w));
		}
		for(ll j=m-1;j>=0;j--){
			auto [y,w]=g[x][j];
			s[j]=s[j+1];
			if(y!=F[0][x])s[j]=merge(s[j],up(h[y],w));
		}
//		cout<<x<<": presuf\n";
//		for(auto [y,w]:g[x])cout<<y<<" ";;cout<<"\n";
//		imp(p);
//		imp(s);
	}
	dfs2(rt);
