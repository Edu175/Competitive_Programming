...
string cv(ln l){return cv(l.p)+"_"+cv(l.p+l.pq);}
int main(){FIN;
	srand(175);
	ll n; cin>>n;
	vector<pt>a;
	...
	vector<ll>per(n);
	fore(i,0,n)cin>>per[i],per[i]--;
	vector<ln>ls;
	fore(i,0,n){
		ls.pb(ln(a[per[i]],a[per[(i+1)%n]]));
	}
//	imp(ls);
	ll flag=1;
	fore(i,0,n)fore(j,i+1,n){
		pt p=ls[i]^ls[j];
		if(p==ls[i].p||p==ls[i].p+ls[i].pq)continue;
		if(p==ls[j].p||p==ls[j].p+ls[j].pq)continue;
		if(ls[i].seghas(p)&&ls[j].seghas(p))flag=0;
//		,cerr<<i<<" "<<j<<"\n";
	}
	if(flag)cout<<"YES\n";
	else cout<<"NO\n";
